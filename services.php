...
if (!empty($keyword)) {
    // If a search keyword is provided, modify the SQL query to filter by services name
    $ambil = $koneksi->query("SELECT * FROM layanan WHERE nama_layanan LIKE '%$keyword%'");
} else {
    // If no search keyword, retrieve all services
...
<html lang="en">
  <head>
    <title>Majestic Pet Care - Layanan</title>
      ...
      <div class="container">
        ...
        <div class="collapse navbar-collapse" id="ftco-nav">
          ...
          <li class="nav-item">
            ...
            <li class="nav-item">
            <?php if (isset($_SESSION["pelanggan"]) && isset($_SESSION["keranjang_layanan"]) && count($_SESSION["keranjang_layanan"]) > 0): ?>
              <a href="keranjang_layanan.php" class="nav-link">Keranjang</a>
              <?php else: ?>
        <a href="services.php" onclick="showAlert()" class="nav-link">Keranjang</a>
    <?php endif; ?>
            </li>
            ...
            <div class="search-form col-md-6">
                <form method="GET" action="services.php">
                    <div class="input-group mb-3">
                        <input type="text" class="form-control" placeholder="Cari layanan..." name="keyword" />
            ...
            <div class="row">
                <?php while ($perproduk = $ambil->fetch_assoc()) { ?>
                    <div class="col-md-3 ftco-animate">
                        <div class="thumbnail">
                            <img src="foto_layanan/<?php echo $perproduk['foto_layanan']; ?>" alt="">
                            <div class="caption">
                                <h3><?php echo $perproduk['nama_layanan']; ?></h3>
                                <h5>Rp. <?php echo number_format($perproduk['harga_layanan']); ?></h5>
                                <a href="detail_layanan.php?id=<?php echo $perproduk['id_layanan']; ?>" class="btn btn-primary">Pesan</a>
                            </div>
                        </div>
                    </div>
                <?php } ?>
  ...
  <script>
    function showAlert() {
        alert("Anda harus memesan layanan terlebih dahulu sebelum ke keranjang");
    }
</script>
