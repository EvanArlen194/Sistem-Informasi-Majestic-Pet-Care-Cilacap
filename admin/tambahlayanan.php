...
<div class="min-height-300 bg-white position-absolute w-100"></div>
  ...
  <section class="py-4">
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <button class="btn btn-green" name="save">Simpan</button>
        <a href="layanan.php" class="btn btn-abu">Kembali</a>
    </form>

    <?php
                if (isset($_POST['save'])) {
                    if (empty($_POST['nama']) || empty($_POST['harga']) || empty($_FILES['foto']['name']) || empty($_POST['deskripsi'])) {
                        echo "<script>alert('Harap isi semua field terlebih dahulu');</script>";
                    } else {
                        $nama = $_FILES['foto']['name'];
                        $lokasi = $_FILES['foto']['tmp_name'];
                        move_uploaded_file($lokasi, "../foto_layanan/" . $nama);
                        $koneksi->query("INSERT INTO layanan (nama_layanan,harga_layanan,foto_layanan,deskripsi_layanan) VALUES('$_POST[nama]','$_POST[harga]','$nama','$_POST[deskripsi]')");
                        echo "<script>alert('Data tersimpan');</script>";
                        echo "<meta http-equiv='refresh' content='1;url=layanan.php'>";
                    }
                }
                ?>
  </div>
